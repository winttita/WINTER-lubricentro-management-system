# Graph Report - Lubricentro  (2026-07-20)

## Corpus Check
- 23 files · ~18,249 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 245 nodes · 443 edges · 35 communities (16 shown, 19 thin omitted)
- Extraction: 100% EXTRACTED · 0% INFERRED · 0% AMBIGUOUS
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `e778bf32`
...
4. `add_movimiento()` - 20 edges
5. `add_producto()` - 16 edges
6. `get_categorias()` - 13 edges
7. `add_categoria()` - 13 edges
8. `get_proveedores()` - 13 edges
9. `add_proveedor()` - 13 edges
10. `_crear_dependencias()` - 13 edges
...
Cohesion: 0.18
Nodes (11): add_cliente(), add_orden_detalle(), add_orden_servicio(), add_servicio(), add_vehiculo(), get_clientes(), get_connection(), get_ordenes() (+3 more)
...
Cohesion: 0.08
Nodes (23): [0.1.0] - 2026-07-16, [0.2.0] - 2026-07-17, [0.2.1] - 2026-07-17, [0.2.2] - 2026-07-17, [0.2.3] - 2026-07-18, [0.2.4] - 2026-07-20, Agregado, Agregado (+15 more)

### Community 5 - "get_movimientos"
Cohesion: 0.09
Nodes (50): add_movimiento(), add_producto(), get_movimientos(), get_productos(), Registra un movimiento de stock y actualiza el stock_actual del producto.     Re, _crear_dependencias(), _crear_producto_con_stock(), Helper para crear un producto con categoría, proveedor y stock inicial (+42 more)

### Community 6 - "_crear_producto_con_stock"
Cohesion: 0.15
Nodes (24): add_categoria(), add_proveedor(), crear_ajuste_stock(), crear_compra(), get_categorias(), get_proveedores(), Crea un ajuste de stock con auditoría.     Inserta el movimiento en la misma tra, Crea una compra a proveedor con múltiples items.     Actualiza stock de cada pro (+16 more)
...
Cohesion: 0.17
Nodes (3): cleanup_old_backups(), get_orden_detalle(), Elimina los backups más antiguos, conservando solo los últimos max_backups.
...
## Knowledge Gaps
- **34 isolated node(s):** `Auto-load Graphify Context on Session Start`, `After Code Changes`, `Quick Queries During Session`, `Project Structure`, `Project Conventions` (+29 more)
...
- **Why does `get_connection()` connect `get_connection` to `get_movimientos`, `_crear_producto_con_stock`, `database.py`, `crear_venta`, `get_venta_completa`, `init_db`, `get_ajustes_stock`, `get_clientes_con_deuda`, `test_add_movimiento_devolucion`, `get_cuenta_corriente_cliente`, `get_movimientos_cuenta_corriente`, `get_reporte_ingresos_egresos`, `get_reporte_inventario`, `get_reporte_ventas`, `get_reporte_ventas_detallado`, `get_ventas`, `update_categoria`, `update_proveedor`, `update_producto`, `update_cliente`, `update_vehiculo`, `update_servicio`, `get_compras`, `get_detalle_compra`?**
  _High betweenness centrality (0.109) - this node is a cross-community bridge._
- **Why does `add_movimiento()` connect `get_movimientos` to `database.py`, `crear_venta`, `get_connection`?**
  _High betweenness centrality (0.046) - this node is a cross-community bridge._
- **Why does `get_productos()` connect `get_movimientos` to `database.py`, `get_connection`, `_crear_producto_con_stock`?**
